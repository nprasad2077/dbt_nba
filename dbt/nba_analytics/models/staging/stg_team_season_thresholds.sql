{{
    config(
        materialized='table',
        schema='staging',
        alias='stg_team_season_thresholds',
        tags=["staging"],
    )
}}

{#
    Computes per-season statistical thresholds (percentiles) for team-level
    advanced metrics.

    These thresholds are used by stg_team_game_adv_stats to dynamically
    categorize team game performances into tiers relative to the distribution
    of their own season, rather than using fixed hardcoded values.

    Grain: one row per season_start_year.

    Metrics covered:
      - Offensive Rating (ortg_*)
      - Defensive Rating (drtg_*)
      - Three-Point Attempt Rate (tpar_*)
      - Assist Percentage / Ball Movement (ast_*)
      - Offensive Rebound Percentage (orb_*)
      - Defensive Rebound Percentage (drb_*)
      - Turnover Percentage (tov_*)
      - Steal + Block Percentage / Defensive Activity (stl_blk_*)
      - True Shooting Percentage (ts_*)
      - Free Throw Rate (ftr_*)
#}
...
SELECT
    ...
    -- =================================================================
    PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY offensive_rating) AS ortg_p5,
    ...
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY offensive_rating) AS ortg_p95,

    -- =================================================================
    -- DEFENSIVE RATING (lower = better defense)
    -- =================================================================
    PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY defensive_rating) AS drtg_p5,
    ...
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY defensive_rating) AS drtg_p95,
    ...
    -- =================================================================
    -- ASSIST PERCENTAGE (ball movement proxy)
    -- =================================================================
    ...
    -- =================================================================
    -- TURNOVER PERCENTAGE (lower = better ball security)
    -- =================================================================
    ...
    -- =================================================================
    -- STEAL + BLOCK COMBINED (defensive activity proxy)
    -- =================================================================

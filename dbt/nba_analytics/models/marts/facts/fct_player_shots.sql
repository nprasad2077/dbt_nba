...
{#
    Grain: One row per individual shot attempt.

    This fact table contains every shot from the shot chart dataset that
    successfully matched to a game_id. Unmatched shots are excluded from
    the mart layer but remain visible in int_player_shots_enriched.
#}

WITH shots_enriched AS (
    SELECT *
    FROM {{ ref('int_player_shots_enriched') }}
    WHERE has_game_match = TRUE  -- Only include shots matched to a game
    {% if is_incremental() %}
...
final AS (
    SELECT
        -- Surrogate Key
        {{ dbt_utils.generate_surrogate_key(['se.shot_id']) }} AS shot_key,

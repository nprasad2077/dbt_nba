...
```bash
docker exec nba_go-postgres-1 pg_dump -U nbago -Fc nba_db > ./2025.11.18_nba_backup.dump
```
...
```bash
pg_restore -U nba_admin -d nba_analytics --no-owner --no-acl ./2025.11.26_nba_backup.dump
```
